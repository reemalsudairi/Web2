...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validate age
    if (empty($age) || !ctype_digit($age) || $age < 18) {
        $errors['age'] = "Please enter a valid age (must be at least 18)!";
    }

    // Validate city
    if (empty($city) || !ctype_alpha(str_replace(" ", "", $city))) {
        $errors['city'] = "Please enter a valid city!";
    }

    // Validate phone number
    if (empty($phone) || !preg_match("/^05\d{8}$/", $phone)) {
        $errors['phone'] = "Please enter a valid phone number (must start with 05)!";
    }

    // Validate bio
    if (empty($bio) || !preg_match("/[a-zA-Z]/i", $bio)) {
        $errors['bio'] = "Please enter a valid bio (must contain letters)!";
    }
    ...
    if (empty(array_filter($errors))) {
        ...
        if (!empty($_FILES['profilepic']['name'])) {
            ...
            $imageName = "images/" . basename($userImage['name']);
            move_uploaded_file($userImage['tmp_name'], $imageName);
        }

        // Call function to handle signup
        if (tutor_signup_handler($fname, $lname, $password, $age, $_POST['gender'], $city, $email, $imageName, $_POST['price'], $bio, $phone)) {
            $errors['notification'] = 'Registration successful!';
            // Clear form fields
            $_POST = array();
                        ...
                        <div class="row">
                            <div class="col-lg-8 col-12 mx-auto">
                                ...
                                <div class="custom-block custom-block-full">
                                    ...
                                    <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" class="custom-form contact-form" enctype="multipart/form-data">
